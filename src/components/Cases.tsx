...
export function Cases({ data, cases }: { data: CasesData; cases: CaseCard[] }) {
  ...
  return (
    <section id="casos" className="bg-brand-black text-white px-6 py-24 md:py-32">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {data?.heading?.lead}{" "}
          <span className="text-white/40">{data?.heading?.dim}</span>
        </SplitReveal>
        <Reveal
          as="p"
          delay={0.1}
          className="mt-8 max-w-2xl text-base md:text-lg text-white/55 leading-relaxed font-light"
        >
          {data?.intro}
        </Reveal>

        <div className="mt-16 md:mt-20 flex flex-col gap-px bg-white/10 border border-white/10">
          {(cases ?? []).map((c) => (
            <Reveal as="div" y={40} key={c._id}>
              <Link
                href={`/casos/${c.slug}`}
                data-cursor="hover"
                className="group relative block bg-brand-black p-8 md:p-12 transition-colors duration-500 hover:bg-white/[0.03]"
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                  ...
                  <div className="lg:col-span-6">
                    ...
                    <p className="text-white/55 leading-relaxed max-w-md">{c.summary}</p>
                    <span className="mt-8 inline-flex items-center gap-2 text-sm font-medium text-white group-hover:text-brand-accent transition-colors">
                      Ver caso completo
                  ...
                  <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-8 lg:border-l lg:border-white/10 lg:pl-12">
                    <div>
                      <span className="block font-mono text-[11px] tracking-[0.25em] uppercase text-white/35 mb-4">
                        El punto de partida
                      </span>
        ...
        {onCta && (
          <Reveal as="div" y={30} className="mt-12">
            <button
              type="button"
              onClick={onCta}
              className="group inline-flex items-center gap-3 px-7 py-3.5 bg-white text-brand-black text-sm font-medium tracking-wide rounded-full"
            >
              {data?.cta?.label}
